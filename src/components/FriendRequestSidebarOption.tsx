...
interface FriendRequestSidebarOptionProps {
  initialUnseenRequest: number;
  sessionId: string;
}

const FriendRequestSidebarOption: FC<FriendRequestSidebarOptionProps> = ({
  sessionId,
  initialUnseenRequest,
}) => {
  const [unseenRequest, setUnseenRequest] =
    useState<number>(initialUnseenRequest);
  useEffect(() => {
  ...
  return (
    ...
    >
      <div className="text-gray-400 border-gray-200 group-hover:border-indigo-600 flex h-6 w-6 shrink-0 items-center justify-center rounded-lg border text-[0.625rem] font-medium ">
